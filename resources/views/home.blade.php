@extends('layouts.app')
@section('title', 'Jadwal Sholat')
@section('user-name', 'Jadwal Sholat KEMENAG')
@section('user-text', 'Indonesia')
@section('user-avatar', 'data:image/jpeg;base64,')
@section('user-text', 'Indonesia')
@push('head')
<link rel="icon" href="data:image/jpeg;base64,">
@endpush
@section('content')
<div class="row">
    <div class="col-md-8 col-md-offset-2 animated fadeInRight">
        <div class="ibox float-e-margins">
            <div class="ibox-title"><h5>Jadwal Sholat <span class="text-navy" id="nama_kota">ACEH BARAT</span> </h5>
              <div class="ibox-tools"> <span class="label label-primary pull-right">By API Fathimah</span></div>
            </div>

            <div class="ibox-content">
               <div class="row">
                  <form id="form" method="post" action="">
                     <div class="col-md-2"></div>
                     <div class="col-md-4">
                        <label for="kota">Pilih Kota</label>
                        <select class="select2_demo_3 form-control" id="city_select" name="kota">
                            <option value="">Kota Yogyakarta</option>
                        </select>
                     </div>
                     <div class="col-md-4">
                        <label for="kota">Pilih Tanggal</label>
                        <div class="input-group date" id="data_1">
                        <span class="input-group-addon" style="border-radius: 5px 0 0 5px"><i class="fa fa-calendar"></i></span><input type="date" value="{{date('Y-m-d')}}"class="form-control" style="border-radius: 0 5px 5px 0" name="tgl">
                        </div>
                     </div>
                     <div class="col-md-2"></div>
                     <button type="submit" id="btn-sub" class="btn btn-primary btn-sx">Cek</button>
                  </form> 
               </div>
               <div class="row" style="margin-top:20px">
                  <div class="col-md-3"></div>
                    <?php
                        $hari = array ( 1 =>    'Senin',
                                    'Selasa',
                                    'Rabu',
                                    'Kamis',
                                    'Jumat',
                                    'Sabtu',
                                    'Minggu'
                                );
                    ?>
                  <div class="col-md-6">
                     <strong id="tanggal">{{$hari[ date('N') ] . date(', d M Y')}}</strong>
                      <div id="vertical-timeline" class="vertical-container dark-timeline">
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon navy-bg">
                                  <i class="fa fa-clock-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Shubuh <span class="text-navy" id="subuh">15:00 WIB</span></h4>
                              </div>
                          </div>
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon red-bg">
                                  <i class="fa fa-sun-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Terbit <span class="text-danger" id="terbit">15:00 WIB</span></h4>
                              </div>
                          </div>
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon navy-bg">
                                  <i class="fa fa-sun-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Dzuhur <span class="text-navy" id="dzuhur">15:00 WIB</span></h4>
                              </div>
                          </div>
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon navy-bg">
                                  <i class="fa fa-clock-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Ashar <span class="text-navy" id="ashar">15:00 WIB</span></h4>
                              </div>
                          </div>
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon navy-bg">
                                  <i class="fa fa-clock-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Maghrib <span class="text-navy" id="maghrib">15:00 WIB</span></h4>
                              </div>
                          </div>
                          <div class="vertical-timeline-block">
                              <div class="vertical-timeline-icon navy-bg">
                                  <i class="fa fa-moon-o"></i>
                              </div>
                              <div class="vertical-timeline-content">
                                    <h4>Isya <span class="text-navy" id="isya">15:00 WIB</span></h4>
                              </div>
                          </div>
                      </div>
                  </div>
                  <div class="col-md-3"></div>
               </div>
            </div>
        </div>
    </div>
</div>
@endsection
@push('body')
    <script>
        $('.select2_demo_3').select2();
        $(document).ready(function(){
            var url = "https://api.banghasan.com/sholat/format/json/kota";
            var city_opt = '';
            var city = [];

            $.get(url, function(data){
                $.each(data.kota, function(key, item){
                    kota = item.nama;
                    id = item.id;
                    if($.inArray(kota,city) === -1){
                        city.push(kota);
                        city_opt += '<option value="'+id+'">'+kota+'</option>'
                    }
                });
            
                $('#city_select').html(city_opt);
            });

            
            $('#form').on('submit', function(event){
                var form = $('#form').serializeArray();
                event.preventDefault();
                var url2 = "https://api.banghasan.com/sholat/format/json/jadwal/kota/"+form[0].value+"/tanggal/"+form[1].value;
                var url1 = "https://api.banghasan.com/sholat/format/json/kota/kode/"+form[0].value;
                var city_opt = '';
                var city = [];

                $.get(url1, function(data){
                    $.each(data.kota, function(key, item){
                        nama = item.nama;

                        $('#nama_kota').text(nama);
                    });
                });

                $.get(url2, function(data){
                    $.each(data.jadwal, function(key, item){
                        Subuh = item.subuh;
                        Terbit = item.terbit;
                        Dzuhur = item.dzuhur;
                        Ashar = item.ashar;
                        Maghrib = item.maghrib;
                        Isya = item.isya;
                        Tanggal = item.tanggal;
                        $('#subuh').text(Subuh+" WIB");
                        $('#terbit').text(Terbit+" WIB");
                        $('#dzuhur').text(Dzuhur+" WIB");
                        $('#ashar').text(Ashar+" WIB");
                        $('#maghrib').text(Maghrib+" WIB");
                        $('#isya').text(Isya+" WIB");
                        $('#tanggal').text(Tanggal);
                    });
                });

            });
        
        });
    </script>
@endpush
